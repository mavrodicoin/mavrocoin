...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "MavrodiCoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"default", "158.69.106.68"}, //First official node
    {"DNS2", "158.69.106.66"}, //second official node
    {NULL, NULL},
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. MavrodiCoin is probably already running."), addrBind.ToString().c_str());
        else
