...
uint256 hashGenesisBlock("0x00000dd19da2995046252528806b6cc0569553f6fdb3097098d87e1a5da9cd48");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MavrodiCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MavrodiCoin Signed Message:\n";
...
{
    // MavrodiCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1)
    {
       nSubsidy = 1000000201 * COIN;
    }
    else if (nHeight <= 101)
    {
        nSubsidy = 2000000 * COIN;
    }
    else if ((nHeight <= 226) && (nHeight > 126))
    {
...
// Launch specs
static const int64 nTargetTimespan = 1 * 60; // MavrodiCoin: each block
static const int64 nTargetSpacing = 1 * 60; // MavrodiCoin: 60 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MavrodiCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, MavrodiCoin - DarkGravity, written by Evan Duffield - evan@MavrodiCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MavrodiCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // MavrodiCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "MavrodiCoin is here";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1487785331;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084459820;
        ...
        assert(block.hashMerkleRoot == uint256("0x3d07ea87ea3e2aae04942bfceee01f18642f28ad6d7a4b4722ab365291b7b0b1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // MavrodiCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MavrodiCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MavrodiCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MavrodiCoinMiner(CWallet *pwallet)
{
    printf("MavrodiCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("MavrodiCoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running MavrodiCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MavrodiCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MavrodiCoinMiner, pwallet));
}
